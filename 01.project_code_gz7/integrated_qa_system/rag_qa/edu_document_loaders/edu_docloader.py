...
if __name__ == '__main__':
    ...
    # 使用 config.DATA_DIR 拼接路径
    file_path = os.path.join(config.DATA_DIR, 'ai_data', '人工智能就业课课程大纲.docx')

    if os.path.exists(file_path):
        docx_loader = OCRDOCLoader(filepath=file_path)
        doc = docx_loader.load()
        print(f"✅ 成功加载 Word 文档，共 {len(doc)} 页")
        for i, page in enumerate(doc):
            print(f"--- 第 {i + 1} 页 ---")
            print(page.page_content[:200])  # 只打印前200字符预览
    else:
        print(f"⚠️ 文件不存在: {file_path}")
        print("请将测试 Word 文件放到 rag_qa/ai_data/edu.docx")